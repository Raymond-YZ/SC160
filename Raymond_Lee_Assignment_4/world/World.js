...
var FSHADER_SOURCE = `
 ...
 void main() {
    ...
    if(u_textureNum == -4){ //skybox specifically to get minimal lighting
      gl_FragColor = u_FragColor;
    }

    else if(u_textureNum == -3){ //use color
      gl_FragColor = vec4((v_Normal + 1.0)/2.0, 1.0);
    }

    else if(u_textureNum == -2){ //use color
      gl_FragColor = u_FragColor;
    }
...
function connectVariablesToGLSL(){
    ...
    camera = new Camera(80, canvas.width/canvas.height, 1, 100);
}
...
function elementUI (){
  ...
  document.getElementById('penguinNormalOn').onclick = function(){ g_penguinNormal=true;}
  document.getElementById('penguinNormalOff').onclick = function(){ g_penguinNormal=false; }
...
function renderScene(){
  ...
  if(g_normal)
    ...
    sphere.textureNum = 0;
  sphere.matrix.translate(1,-0.5,-1);
